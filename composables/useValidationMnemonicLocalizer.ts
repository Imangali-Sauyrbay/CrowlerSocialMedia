...
const minRuleLocalizer = (message: string, localizedField: string): string => {
    if (message.startsWith("min-string")) {
        ...
        return i18n.global.t("validation.rules.min-string", {
...
const containRuleLocalizer = (
    ...
    if (toContain.startsWith("special-character"))
        return i18n.global.t("validation.rules.must-contain-characters", {
            field: localizedField,
            count: 1,
    ...
    if (toContain.startsWith("uppercase"))
        return i18n.global.t("validation.rules.must-contain-uppercase", {
            field: localizedField,
            count: 1,
        });

    if (toContain.startsWith("number"))
        return i18n.global.t("validation.rules.must-contain-number", {
            field: localizedField,
            count: 1,
        });
...
const localizeErrorMessage = (
    ...
    localizedField =
        localizedField || i18n.global.t("validation.fields.default");
    ...
    if (message === "email") return i18n.global.t("validation.rules.email");

    return i18n.global.t("validation.rules." + message, {
        field: localizedField,
    });
};
