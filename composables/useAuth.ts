...
export interface RefreshResponce extends LoginResponce {
    exp: number;
}

const isUnauthorizedError = (e: unknown) =>
    e instanceof FetchError && e.status === StatusCodes.UNAUTHORIZED;
...
export const useDefaultLogo = () => {
    ...
    const withDefaultLogo = (logo: string | null | undefined): string =>
...
const useAuthInitializing = () =>
    useState<boolean>("auth:initializing", () => true);
const useRefreshTimeoutID = () => useState<number>("auth:refresh-timeout");

const setUser = (value: UnwrapRef<ReturnType<typeof useAuthUser>>) =>
    (useAuthUser().value = value);
const setToken = (value: string | null) => (useAuthToken().value = value);
const setAuthInitializing = (value: boolean) =>
    (useAuthInitializing().value = value);
...
const refreshToken = async () => {
    return setAuthResponce(await $fetch<RefreshResponce>("/api/auth/refresh"));
};
