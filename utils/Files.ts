import { DEFAULT_UPLOAD_FILES_OPTION, IMAGE_TYPES, GIF_TYPES } from "~/constants/FILES"
...
export const readImageFileAsDataUrl = (file: File) => {
    return new Promise<string>((resolve, reject) => {
        if(!isImage(file) && !isGif(file))
            return reject('Function readImageFile reads only images!')
        ...
        fileReader.onload = (e) => {
            if(! fileReader.result)
                return reject('Unexpected error occured while reading ' + file.type + '!')
...
export const isFilesSatisfiesCount = (files: FileList | File[], options: isFilesSatisfiesCountOptions = DEFAULT_UPLOAD_FILES_OPTION): boolean => {
    const types: string[] = []
...
export const getMergedAndValidatedFiles = (target: HTMLInputElement, files: File[]) => {
